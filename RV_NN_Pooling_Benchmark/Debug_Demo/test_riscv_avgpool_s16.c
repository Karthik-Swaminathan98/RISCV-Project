...
#include "../TestData_ARM/avgpooling_int16_1/test_data.h"
#include "../TestData_ARM/avgpooling_int16_2/test_data.h"
...
void avgpooling_int16_riscv_avgpool_s16(void)
{
    ...
    if (validate_s16(output,
		...
		printf("Stack Used for riscv_avgpool_s16: %lu bytes\n\r", (unsigned long)stack_used_s16);
...
void avgpooling_int16_1_riscv_avgpool_s16(void)
{
    ...
    ctx.size = riscv_avgpool_s16_get_buffer_size(AVGPOOLING_INT16_1_OUTPUT_W, AVGPOOLING_INT16_1_INPUT_C);
    ctx.buf = malloc(ctx.size);
    ...
    {
        printf("riscv_avgpool_s16 (1) output validation PASSED\n\r");
        printf("Stack Used for riscv_avgpool_s16 (1): %lu bytes\n\r", (unsigned long)stack_used_s16);
        printf("Cycle for riscv_avgpool_s16 (1): %lu\n\r", (unsigned long)cycle_count_s16);
    }
    else
    {
        printf("riscv_avgpool_s16 (1) output validation FAILED\n\r");
    }
}
...
void avgpooling_int16_2_riscv_avgpool_s16(void)
{
    ...
    ctx.size = riscv_avgpool_s16_get_buffer_size(AVGPOOLING_INT16_2_OUTPUT_W, AVGPOOLING_INT16_2_INPUT_C);
    ctx.buf = malloc(ctx.size);
    ...
    {
        printf("riscv_avgpool_s16 (2) output validation PASSED\n\r");
        printf("Stack Used for riscv_avgpool_s16 (2): %lu bytes\n\r", (unsigned long)stack_used_s16);
        printf("Cycle for riscv_avgpool_s16 (2): %lu\n\r", (unsigned long)cycle_count_s16);
    }
    else
    {
        printf("riscv_avgpool_s16 (2) output validation FAILED\n\r");
    }
}

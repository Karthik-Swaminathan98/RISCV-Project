...
void benchmark_sqrt(void) {
    ...
    for (int i = 0; i < FIR_SIZES_COUNT; i++) {
        ...
        // Inputs: ramp [0..1]
        for (int j = 0; j < N; j++) {
            float32_t x    = (float32_t)j / (float32_t)(N - 1); // Normalized ramp 0 to 1
            in_f32[j]      = x;                                // Store as floating point
            in_q15[j]      = (q15_t)(x * 0x7FFF);              // Convert to Q15 format
        }
        ...
        for (int j = 0; j < N; j++) {
            out_f32[j] = riscv_dsp_sqrt_f32(in_f32[j]);  // Direct result assignment
        }
        ...
        for (int j = 0; j < N; j++) {
            out_q15[j] = riscv_dsp_sqrt_q15(in_q15[j]);  // Direct result assignment
        }
        ...
        // Optional: Verification for N=32
//        if (N == 32) {
//            printf("\n--- Q15 Functional Verification (N=32) ---\n\r");
//            printf("Input (as float), Output (as float)\n\r");
//            for (int k = 0; k < N; k++) {
//                // Convert Q15 values back to float for printing

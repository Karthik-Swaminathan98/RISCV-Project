...
// Input data arrays
float32_t original_input[2 * N];     // Array to store the original input signal
float32_t input[2 * N];              // Array for processing FFT
float32_t magnitude[NUM_EXECUTIONS][N]; // Magnitude results for each execution
float32_t frequency_bin[N];          // Frequency bin array
unsigned int cycle_counts[NUM_EXECUTIONS]; // Array to store cycle counts
float32_t magnitude_temp[N];         // Temporary array to store magnitude results
...
int main(void) {
    ...
    user_init();
    delay_ms(20);
    printf("Starting F32 FFT Benchmark \n");
    delay_ms(20);

    // Generate a sine wave signal in F32 format and store it in the original input array
    for (int i = 0; i < N; i++) {
        original_input[2 * i] = sinf(2 * M_PI * SIGNAL_FREQ * i / SAMPLING_FREQ); // Real part in F32 format
    ...
    for (int execution = 0; execution < NUM_EXECUTIONS; execution++) {
        ...
        riscv_cmplx_mag_f32(input, magnitude_temp, N);
        for (int i = 0; i < N; i++) {
    ...
    for (int i = 1; i < NUM_EXECUTIONS; i++) {
        for (int j = 0; j < N; j++) {
            if (fabsf(magnitude[0][j] - magnitude[i][j]) > 1e-5) { // Tolerance for F32 format
                consistent = 0;
                break;
            }
        }
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int execution = 0; execution < NUM_EXECUTIONS; execution++) {
            printf("\t%.5f", magnitude[execution][i]); // No Q15_SCALE division needed
        }
        printf("\n");
        delay_ms(30);
    }
